...
if [ $(echo `pwd` | grep setup_scripts | wc -l) -ne 0 ]; then
	...
	# place spigot alongside npcss
	mkdir -p ../spigot
	cd ../spigot
	spigot_dir=`pwd`
	rm $spigot_file 2> /dev/null
	wget http://jenkins.npcss.co/job/spigot/lastSuccessfulBuild/artifact/Spigot/Spigot-Server/target/$spigot_file
	...
	echo 'Creating the plugins directory and linking the npcss plugin'
	mkdir -p plugins
	rm plugins/npcss-all.jar 2> /dev/null
	ln -s $npcss_dir/build/libs/npcss-all.jar $spigot_dir/plugins/npcss-all.jar
